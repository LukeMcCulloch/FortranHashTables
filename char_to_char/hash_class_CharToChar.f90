...
MODULE hash_chtch_class
  IMPLICIT NONE
  



  type, public :: hash_chtch
  ...
  interface hash_chtch
     module procedure hash_init
  end interface hash_chtch
...
CONTAINS
  ...
  SUBROUTINE hash_push(self, key, value)
    class(hash_chtch), intent(inout) :: self 
  ...
  SUBROUTINE hash_set(self, key, value)
    ...
    DO local_index = 1,Size(self%keys,1)
      IF(TRIM(self%keys(local_index)) == TRIM(key)) THEN
  ...
  SUBROUTINE hash_print(self)
    ...
    DO local_index = 1,Size(self%keys,1)
       IF(self%used(local_index)) PRINT*, TRIM(self%keys(local_index)),&
  ...
  SUBROUTINE hash_reallocate(self)
    class(hash_chtch), intent(inout) :: self
    CHARACTER(self%CharLength), ALLOCATABLE :: temp_keys(:)
